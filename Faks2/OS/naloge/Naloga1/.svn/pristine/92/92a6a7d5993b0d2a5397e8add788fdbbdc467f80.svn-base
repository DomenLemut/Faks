...
int main(int argc, char *argv[])
{
	...
	while (active)
	{
		...
		if (tokenise(inBuffer) > 0)
		{
			...
			// 	if (tokens[i][0] == '>')
			// 	{
			// 		strcpy(outPath, &tokens[i][1]);
			// 		printf("outPath: %s\n", outPath);
			// 		outFd = open(outPath, O_CREAT | O_TRUNC | O_WRONLY, S_IRUSR | S_IWUSR);
			// 		if (outFd < 0)
			// 		{
...
int pipes()
{
	...
	char pipeTokens[numTokens][TOKENS_SIZE];
	int numTokensSave = numTokens;

	// prepisi vse, ob tokenise se tokens prepise z novimi podatki, spremeni se tudi numTokens
	for (int i = 0; i < numTokens; i++)
		strcpy(pipeTokens[i], tokens[i]);
...
int createPipe()
{
	...
	if (pid1 == 0)
	{
		...
		char *cmd2[] = {tokens[2], NULL};
		return execvp(tokens[0], cmd2);
	}
	else
	{
		pid2 = fork();
