	...
	while (active)
	{
		...
		if (tokenise(inBuffer) > 0)
		{
			...
			for (; i < numTokens; i++)
			{
				if (tokens[i][0] == '<')
				{
					char *file = strtok(tokens[i], "<");
					file = strtok(NULL, " ");
					file = strtok(file, "<");

					inputRedirect = open(file, O_RDONLY);
					if (inputRedirect == -1)
					{
					...
					if (dup2(inputRedirect, STDIN_FILENO) == -1)
					{
				...
				else if (tokens[i][0] == '>')
				{
					...
					outputRedirect = open(file, O_WRONLY | O_CREAT | O_TRUNC, 0644);
					if (outputRedirect == -1)
					{
					...
					if (dup2(outputRedirect, STDOUT_FILENO) == -1)
					{
