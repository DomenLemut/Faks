		...
		if (isdigit(entry->d_name[0]))
		{
			...
			if (fd != -1)
			{
				...
				if (bytesRead > 0)
				{
					...
					sscanf(buffer, "%d %*c %s %c %d", &pid, name,
						   &x[0], &ppid);
					x[1] = '\0';
...
// 	char pipeTokens[NUM_TOKENS][TOKENS_SIZE];
// 	int numTokensSave = numTokens - 1;

// 	for (int i = 0; i < numTokensSave; i++)
// 		strcpy(pipeTokens[i], tokens[i + 1]);

// 	for (int i = 0; i < numTokensSave; i++)
// 	{
// 		tokenise(pipeTokens[i]);
...
int pipes()
{
	...
	int commandIndex = 0;
	int inputRtrn = STDIN_FILENO;
	int outputRtrn;

	while (commandIndex < numCommands)
	{
		tokenise(pipeTokens[commandIndex]);

		if (commandIndex == numPipes)
			outputRtrn = STDOUT_FILENO;
		else
			outputRtrn = fds[(commandIndex * 2) + 1];
		...
		{
			if (dup2(inputRtrn, STDIN_FILENO) < 0)
			{
				perror("dup2");
				return EXIT_FAILURE;
			}

			if (dup2(outputRtrn, STDOUT_FILENO) < 0)
			{
				perror("dup2");
				return EXIT_FAILURE;
			}
