...
int tokenise(char *input)
{
	...
	int numTokens = 0; // Declare and initialize numTokens variable

	while (i < strLen)
	{
		currTokenPlace = 0;

		while (isspace((unsigned char)input[i]) && i < strLen)
			i++;
...
int pinfo_n()
{
	...
	while ((entry = readdir(dir)) != NULL)
	{
		if (isdigit(entry->d_name[0]))
		{
			...
			char filepath[20000];
			snprintf(filepath, sizeof(filepath), "%s/%d/stat", path1, pid);
