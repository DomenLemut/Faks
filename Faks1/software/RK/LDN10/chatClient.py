...
def handle_msg(sock, msg):
    ...
    if msg[0:3] == "RNR":
        if msg[3:] != "0":  #good system acceted our rename
            ...
            print("---------------------------------------------------------------------")
            print(" \n \n [System] Uporabnik zvasim certifikatom je ze prijavljen")
...
def handle_command(sock, comm):
    ...
    if comm[0] != "!":
        ...
        if "!help" in comm.lower():
            ...
            print("![ime] spremeni vas chat tako da bodo sporocila poslana OSEBI")
            print("\n!@ spremeni vas chat tako da bodo sporocila poslana VSEMi")
            print("-------------------- \n")
        if(comm[1] == "@"):
            user_bind = "@"
        else:
